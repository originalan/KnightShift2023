...
@Autonomous(name = "Red2_2T_1P (places pixel, parks inner)", group = "Autonomous Opmode 11.19")
public class Red2_2T_1P extends AutoBase {
        ...
        switch (detectedSide) {
            case LEFT:
                detectionTraj = drive.trajectorySequenceBuilder(startingPose)
                        .splineTo(new Vector2d(-45.5 + 1.5, -29.5), Math.toRadians(270))
            ...
            case MIDDLE:
                detectionTraj = drive.trajectorySequenceBuilder(startingPose)
                        .splineTo(new Vector2d(-35.25 + 1.5, -25.0), Math.toRadians(270))
                        // Center of robot, adjusted so purple pixel servo is in line with the center
                        .UNSTABLE_addDisplacementMarkerOffset(AUTO_PURPLE_PIXEL_RELEASE, () -> {
                            robot.purplePixel.drop();
                        })
